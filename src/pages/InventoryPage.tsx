            ...
            {loading ? (
              ...
              <>
                ...
                {filtered.length === 0 ? (
                  ...
                  <div className={viewMode === "grid" ? "grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6" : "space-y-3 sm:space-y-4"}>
                    {paginated.map((car) => (
                      <Link
                        to={`/inventory/${car.id}`}
                        key={car.id}
                      ...
                      >
                        <div className={`bg-muted flex items-center justify-center overflow-hidden shrink-0 ${viewMode === "list" ? "w-full sm:w-56 h-40 sm:h-48" : "h-32 sm:h-56"}`}>
                          {car.image_url ? (
                            ...
                            <Car className="h-10 w-10 sm:h-16 sm:w-16 text-muted-foreground/30" />
                          )}
                        </div>
                        <div className="p-3 sm:p-5 flex flex-col justify-between flex-1">
                          <div>
                            <h3 className="font-heading font-bold text-sm sm:text-lg text-foreground mb-0.5 sm:mb-1 group-hover:text-primary transition-colors line-clamp-1">{car.name}</h3>
                            <p className="text-primary font-semibold text-xs sm:text-base mb-1.5 sm:mb-3">{formatPrice(car.price)}</p>
                            <div className="flex flex-wrap items-center gap-1.5 sm:gap-3 text-[10px] sm:text-xs text-muted-foreground">
                              ...
                              {car.mileage && <span className="hidden sm:flex items-center gap-1"><Gauge className="h-3 w-3" />{car.mileage}</span>}
                              {car.fuel && <span className="flex items-center gap-1"><Fuel className="h-3 w-3" />{car.fuel}</span>}
                              {car.transmission && <span className="hidden sm:inline bg-muted px-1.5 sm:px-2 py-0.5 rounded text-muted-foreground">{car.transmission}</span>}
                              {car.body_type && <span className="hidden sm:inline bg-muted px-1.5 sm:px-2 py-0.5 rounded text-muted-foreground">{car.body_type}</span>}
                              {car.engine_cc && <span className="hidden sm:inline bg-muted px-1.5 sm:px-2 py-0.5 rounded text-muted-foreground">{car.engine_cc} CC</span>}
                            </div>
                          </div>
                          <div className={`flex items-center gap-3 sm:gap-4 mt-2 sm:mt-3 ${viewMode === "list" ? "sm:mt-0" : ""}`}>
                            {viewMode === "list" && (
                            ...
                            <button
                              ...
                              className="inline-flex items-center gap-1 text-xs sm:text-sm font-medium text-primary hover:underline focus:outline-none"
